...
export async function getServerSideProps({ params }) {
  ...
  try {
    const [histRes,bestRes,snapRes,liveRes] = await Promise.all([
      ...
      // Query dedicada: snapshot más reciente SIN filtro de fecha, para estado en vivo real
    ...
    // El historial se muestra cronológico (más viejo primero), pero lo pedimos desc para no perder el snapshot actual con limit=1000
    ...
    // latestSnap: snapshot más reciente, independiente del historial de 30 días
    const latestSnap = Array.isArray(liveRows) ? liveRows[0]||null : null;
...
export default function ModelPage({ username,history,bestHours,country,gender,displayName,languages,similarModels,latestSnap }) {
  ...
  // isLive: usa el snapshot más reciente real (no el último del historial paginado)
  // Considera "en vivo" si tiene viewers > 0 y el snapshot fue hace menos de 3 horas
  ...
  // Para mostrar viewers actuales, priorizar latestSnap
  const currentViewers = isLive ? liveViewers : (viewers != null && viewers > 0 ? viewers : null);
