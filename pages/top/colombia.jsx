...
const COUNTRY_NAMES = {
  ...
  ES:"España",US:"Estados Unidos",BR:"Brasil",RO:"Rumania",RU:"Rusia",DE:"Alemania",
};
...
export async function getServerSideProps() {
  ...
  // 1. Chaturbate API en tiempo real — filtrar solo Colombia
  try {
    ...
    if (res.ok) {
      ...
      const colombianas = results.filter(r => (r.country||"").toUpperCase() === "CO" || r.username === LEXY);
      if (colombianas.length > 0) {
        isLiveData = true;
        models = colombianas
  ...
  if (models.length === 0 && SUPABASE_URL && SUPABASE_KEY) {
    try {
      ...
      const url = `${SUPABASE_URL}/rest/v1/rooms_snapshot?captured_at=gte.${since}&country=eq.CO&num_users=gt.0&select=username,display_name,num_users,country,gender&order=num_users.desc&limit=200`;
  ...
  if (!models.some(m => m.username === LEXY)) {
    ...
    if (SUPABASE_URL && SUPABASE_KEY) {
      try {
        const since = new Date(Date.now()-26*60*60*1000).toISOString();
        const r = await fetch(`${SUPABASE_URL}/rest/v1/rooms_snapshot?username=eq.${LEXY}&captured_at=gte.${since}&select=username,display_name,num_users&order=num_users.desc&limit=1`,
...
export default function TopColombiaPage({ models, isLiveData, fetchedAt }) {
  ...
  const pageTitle = liveCount > 0
    ? `Top ${liveCount} Colombianas en Chaturbate en vivo ahora 🇨🇴 | CampulseHub`
    : `Top Colombianas en Chaturbate — Modelos de Colombia en vivo | CampulseHub`;
  const pageDescription = `Las ${liveCount||"mejores"} modelos colombianas en vivo en Chaturbate ahora mismo.${topViewers>0?` La más vista tiene ${topViewers.toLocaleString("es")} viewers en este momento.`:""} Viewers reales, actualizados en tiempo real.`;

  const schema = {
    "@context":"https://schema.org","@type":"CollectionPage",name:pageTitle,description:pageDescription,
    url:`${SITE}/top/colombia`,
    breadcrumb:{"@type":"BreadcrumbList",itemListElement:[
      {"@type":"ListItem",position:1,name:"CampulseHub",item:SITE},
      {"@type":"ListItem",position:2,name:"Top Colombia 🇨🇴",item:`${SITE}/top/colombia`}
    ]},
    hasPart:ordered.slice(0,10).map(m=>({
      "@type":"WebPage",name:`${m.display_name||m.username} — modelo colombiana en Chaturbate`,url:`${SITE}/model/${m.username}`
    })),
  };
  ...
  return (
    <>
      <Head>
        ...
        <meta name="keywords" content="colombianas chaturbate, modelos colombianas en vivo, chaturbate colombia, webcam colombia, colombianas online"/>
        <meta name="robots" content="index, follow"/>
        <link rel="canonical" href={`${SITE}/top/colombia`}/>
        <meta name="twitter:card" content="summary_large_image"/>
        <meta name="twitter:title" content={pageTitle}/>
        <meta name="twitter:description" content={pageDescription}/>
        <meta property="og:title" content={pageTitle}/>
        <meta property="og:description" content={pageDescription}/>
        <meta property="og:url" content={`${SITE}/top/colombia`}/>
      ...
      <div className="cmp-page cmp-page-body">
        ...
        <nav className="cmp-bc">
          ...
          <a href="/country/co" style={{fontWeight:500,textDecoration:"none",color:"var(--txt2)"}}>Colombia</a>
          <span className="cmp-bc-sep">›</span>
          <span style={{color:"var(--txt)"}}>Top Colombianas</span>
        </nav>

        <div className="cmp-page-header">
          <h1 className="cmp-page-h1">🇨🇴 Top Colombianas en vivo</h1>
          <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:".75rem",flexWrap:"wrap"}}>
            ...
            {topViewers > 0 && <span style={{background:"rgba(56,182,212,.1)",color:"var(--neon)",fontSize:".75rem",padding:"3px 12px",borderRadius:20,border:"1px solid rgba(56,182,212,.25)",fontWeight:600}}>
              Máx. {topViewers.toLocaleString("es")} viewers
            </span>}
          ...
          <p className="cmp-page-sub">Las mejores modelos colombianas de Chaturbate ordenadas por viewers en tiempo real.</p>
        </div>

        {/* Links a otras categorías relacionadas */}
        <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:"1.5rem"}}>
          {["MX","AR","CL","ES","PE"].map(c=>(
              ...
              return (
                <a key={m.username} href={`/model/${m.username}`}
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{fontSize:".75rem",color:"var(--txt3)",marginTop:2}}>🇨🇴 Colombia</div>
                  </div>
        ...
        <section style={{marginTop:48,padding:"1.5rem",background:"var(--surf)",borderRadius:14,border:"1px solid var(--bdr)"}}>
          ...
          <p style={{color:"var(--txt2)",fontSize:".875rem",lineHeight:1.7}}>
            Colombia es uno de los países con más modelos activas en Chaturbate. CampulseHub rastrea en tiempo real las estadísticas de todas las colombianas en vivo: viewers, seguidores, mejores horarios y más.
          </p>
          <p style={{color:"var(--txt2)",fontSize:".875rem",lineHeight:1.7,marginTop:".75rem"}}>
            Los datos se actualizan automáticamente cada hora. Puedes filtrar por <a href="/country/co" style={{color:"var(--neon)"}}>ciudad en Colombia</a> o explorar el <a href="/top/latinas" style={{color:"var(--neon)"}}>ranking completo de latinas</a>.
          </p>
