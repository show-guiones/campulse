...
const COUNTRY_NAMES = { CO:"Colombia",MX:"México",AR:"Argentina",CL:"Chile",ES:"España",PE:"Perú" };
...
export async function getServerSideProps() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      const espanolas = results.filter(r => (r.country||"").toUpperCase() === "ES");
      if (espanolas.length > 0) {
        isLiveData = true;
        models = espanolas
          .map(r => ({ username:r.username, display_name:r.display_name||r.username, num_users:r.num_users??0, country:"ES", gender:r.gender||"" }))
          .sort((a,b)=>b.num_users-a.num_users).slice(0,50);
      }
    }
  } catch (_) {}
  ...
  if (models.length === 0 && SUPABASE_URL && SUPABASE_KEY) {
    try {
      ...
      const url = `${SUPABASE_URL}/rest/v1/rooms_snapshot?captured_at=gte.${since}&country=eq.ES&num_users=gt.0&select=username,display_name,num_users,country,gender&order=num_users.desc&limit=200`;
      ...
      if (r.ok) {
        ...
        models = [...map.values()].sort((a,b)=>b.num_users-a.num_users).slice(0,50).map(r=>({...r,country:"ES",fromCache:true}));
      }
    } catch (_) {}
  }
...
export default function TopEspanaPage({ models, isLiveData }) {
  ...
  const pageTitle = liveCount > 0
    ? `Top ${liveCount} Españolas en Chaturbate en vivo ahora 🇪🇸 | CampulseHub`
    : `Top Españolas en Chaturbate — Modelos de España en vivo | CampulseHub`;
  const pageDescription = `Las ${liveCount||"mejores"} modelos españolas en vivo en Chaturbate ahora mismo.${topViewers>0?` La más vista tiene ${topViewers.toLocaleString("es")} viewers.`:""} Datos actualizados en tiempo real.`;
  ...
  const schema = {
    ...
    breadcrumb:{"@type":"BreadcrumbList",itemListElement:[
      ...
      {"@type":"ListItem",position:2,name:"Top España 🇪🇸",item:`${SITE}/top/espana`}
    ]},
    hasPart:models.slice(0,10).map(m=>({ "@type":"WebPage",name:`${m.display_name||m.username} — modelo española en Chaturbate`,url:`${SITE}/model/${m.username}` })),
  };
  ...
  return (
    <>
      <Head>
        ...
        <meta name="keywords" content="españolas chaturbate, modelos españolas en vivo, chaturbate españa, webcam españa, españolas online"/>
        <meta name="robots" content="index, follow"/>
        <link rel="canonical" href={`${SITE}/top/espana`}/>
        <meta name="twitter:card" content="summary_large_image"/>
        <meta name="twitter:title" content={pageTitle}/>
        <meta name="twitter:description" content={pageDescription}/>
        <meta property="og:title" content={pageTitle}/>
        <meta property="og:description" content={pageDescription}/>
        <meta property="og:url" content={`${SITE}/top/espana`}/>
      ...
      <div className="cmp-page cmp-page-body">
        ...
        <nav className="cmp-bc">
          ...
          <a href="/country/es" style={{fontWeight:500,textDecoration:"none",color:"var(--txt2)"}}>España</a>
          <span className="cmp-bc-sep">›</span>
          <span style={{color:"var(--txt)"}}>Top Españolas</span>
        </nav>

        <div className="cmp-page-header">
          <h1 className="cmp-page-h1">🇪🇸 Top Españolas en vivo</h1>
          ...
          <p className="cmp-page-sub">Las mejores modelos españolas de Chaturbate ordenadas por viewers en tiempo real.</p>
        </div>

        <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:"1.5rem"}}>
          {["CO","MX","AR","CL","PE"].map(c=>(
              ...
              return (
                <a key={m.username} href={`/model/${m.username}`}
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{fontSize:".75rem",color:"var(--txt3)",marginTop:2}}>🇪🇸 España</div>
                  </div>
        ...
        <section style={{marginTop:48,padding:"1.5rem",background:"var(--surf)",borderRadius:14,border:"1px solid var(--bdr)"}}>
          ...
          <p style={{color:"var(--txt2)",fontSize:".875rem",lineHeight:1.7}}>
            España es el país con más clicks en CampulseHub. Aquí encuentras las modelos españolas más vistas en Chaturbate ahora mismo, con estadísticas actualizadas cada hora.
          </p>
